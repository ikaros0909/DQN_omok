...
#DQN 에이전트
class DQNAgent:
	def __init__(self, action_size, stone_color):		
		...
		if self.load_model:
			if stone_color=="black":
				self.model.load_weights("./save_model/omok_black.h5")
			else:
				self.model.load_weights("./save_model/omok_white.h5")
    ...
    # 입실론 탐욕 정책으로 행동 선택
	def get_action(self, history, _move, _board_no):
		co=0
		di=2
		if np.random.rand() <= self.epsilon:
			...
			Qv = np.array(q_value[0])
			Qv = np.argsort(Qv)
			
			for _i in range(224,-1, -1):
				_action = Qv[_i]
				#print(_move, _i, _action, q_value[0,_action], np.argmax(q_value), q_value[0, np.argmax(q_value)])
				if _move == 1:
					return _action
				
				if _board_no[_action//15,_action%15] == 0:
...
if __name__ == "__main__":
	...
	agent = [DQNAgent(action_size=225,stone_color="white"),DQNAgent(action_size=225,stone_color="black")]
	...
	for e in range(EPISODES):
		...
		for move in range(1, 226):
			...
			if done == False and op_done == False:
				action[move%2] = agent[move%2].get_action(state, move, board_no)
			...
			if done == True:				
				...
				agent[(move+1)%2].append_sample(undo_state, action[(move+1)%2], -100, state)
			elif move > 1:
				agent[(move+1)%2].append_sample(undo_state, action[(move+1)%2], 0, state)
				if move == 225:
			...
			if global_step % agent[(move+1)%2].update_target_rate == 0:
		...
		if e % 100 == 0:
			...
			print(board_no)
			print("episode:", e, "  move:",move,"  black:",black,"  white:",white)
			print("time:",now1,"{",now1-now,"}")
			now = now1
			agent[0].model.save_weights("./save_model/omok_white.h5")
			agent[1].model.save_weights("./save_model/omok_black.h5")
