...
class DQNAgent:
	def __init__(self, action_size, stone_name):		
		...
		# DQN 하이퍼파라미터
		self.epsilon = 1.
		self.epsilon_start, self.epsilon_end = 1.0, 0.005
...
def color_change(_state, _move):
	temp_state = np.copy(_state)
	_state[:,:,:,0:7] = temp_state[:,:,:,8:15]
	_state[:,:,:,8:15] = temp_state[:,:,:,0:7]
	_state[:,:,:,1:7] = temp_state[:,:,:,0:6]
	_state[:,:,:,9:15] = temp_state[:,:,:,8:14]
	_state[:,:,:,16] = (_move+1)%2
...
if __name__ == "__main__":
	...
	for e in range(EPISODES):
		...
		for move in range(1, 226):
			...
			if done == True:
				#백이 졌을 때, 수순이 많이 진행 될 수록 벌점을 줄임
				if move%2 == 0:
					reward = -1000
				else:
					reward = -1000/move
				
				#오목이 만들어 진경우 해당 에이전트는 현재 상태, 현재 액션, 리워드, 착수상태 저장
				agent[(move)%2].append_sample(state, action[(move)%2], 225/move, move_state)				
				#다른 에이전트는 이전 상태, 이전 액션, 리워드, 색이 변경된 현재 착수 상태 저장
				color_change(move_state,move)
				agent[(move+1)%2].append_sample(undo_state, action[(move+1)%2], reward, move_state)
				
			elif move > 1:				
				#두번째 수 이후 부터 
				#현재 착수 상태의 색을 변경 후 다른 에이전트의 이전 상태, 이전 액션, 리워드, 색이 변경된 현재 착수 상태 저장		
				color_change(move_state,move)				
				agent[(move+1)%2].append_sample(undo_state, action[(move+1)%2], 0, move_state)
				#이전 상태를 현재 상태로 변경
				undo_state = np.copy(state)
				#현재 상태를 색이 변경된 착수 상태로 저장
